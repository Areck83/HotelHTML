...
<div class="container">
  <div class="row">
	...
	$result=mysqli_query($nombreConexion,"select * from usuario");
	?>
	...
	/* Se determina el n�mero de registros recuperados por el cursor*/
      $numeroDeusuarios=mysqli_num_rows($hacerConsulta);
      echo "<p style='color:white; font-size:24px'>Total de usuarios: $numeroDeusuarios";
    ?>

    <form action="" method="post" name="" id="">
	  <table cellspacing = "1px" cellpadding = "10px">
   	<tr class="encabezado"><th>Nombre</th><th>Usuario</th><th>Alias</th><th>Correo</th></tr>

      <?php
          while($row = mysqli_fetch_array($result)) {
           printf("<tr><td>%s</td><td>%s</td><td>%s</td><td>%s</td>",
	    	$row["NombreC"],$row["NombreU"],$row["Alias"],$row["Correo"]);
          }
      ?>
	</table>
    </form>
...
{
$name=$_POST['NombreU'];
mysqli_query($nombreConexion,"delete from usuario where NombreU = '$name';" );
}
?>
...
$rs = mysqli_query($nombreConexion,$q);
while ($row = mysqli_fetch_array($rs)) {
		echo "<option value='".$row['NombreU']."'";
		echo"SELECTED";
		echo ">";
		echo $row['NombreU'];
		echo "</option>";;
...
<input type="submit" name="Borrar" value="Borrar Usuario" id="delete"/>
</form>
